...
\begin{theorem}
  Olkoon $x,y \in \mathbb{R}$. Nyt

  $x \lor y = \max \{x,y\} = \nicefrac{1}{2}(x + y + |x-y|)$ ja

  $x \land y = \min \{x,y\} = \nicefrac{1}{2}(x + y - |x-y|)$.
\end{theorem}

\begin{theorem}
  Jos $f : [a,b] \rightarrow \mathbb{R}$ jva, niin $\exists M < \infty$ s.t.
  $|f(x)| \leq M$ $\forall x \in [a,b]$.
\end{theorem}

\newtheorem*{väliarvolause}{Väliarvolause}
\begin{väliarvolause}
  Olk $f$ dva välillä $[a,b]$. Tällöin $\exists \xi \in ]a,b[$ s.e.  $f'(\xi) =
  \frac{f(b) - f(a)}{b-a}$.
\end{väliarvolause}
...
\section{Muuta}
\begin{itemize}
  \item{$\bar{\mathbb{Q}} = \overline{\mathbb{Q}^C} = \mathbb{R}$}
  \item{$A,B \cls \mathbb{R} \implies A \times B \cls \mathbb{R}^2$}
  \item{Avaruudessa $C[a,b]$ sisätulo $f \cdot g = \int_{a}^{b} f(x) g(x) dx$}
  \item{$\|x\|_0 = \max \{ |x_j| \: : \: 1 \leq j \leq n \}$}
\end{itemize}
